//Group Work Tyler Speegle, Taylor Dunn
...
{
  return (0x1& (input >> b)); //Takes the input, shifts it b to the right. That puts the value we care about in the initial position. Then take 0x1& which will change everything to 0 except the last value. If the value is 1, it returns 1 (which is also true) and if it's a 0, it will return 0 (which is false). Definitely a little "magical" but it works and makes sense in my mind haha.
}
...
{
    return ((input>> (b*8))); //because char is 8 bits, it will only look at the the first byte.
                              //Could 0xFF & to change everything to 0 except your last byte
}
...
uint32_t SetByte(uint32_t input, uint8_t value, int b){
    uint32_t endResult = 0x0;
    uint32_t valueHolder = value;
    uint32_t leftSide = input >> (8*b);
    uint32_t rightSide = input << 8*(3-b);
    leftSide = leftSide & 0xFFFFFF00;
...
//    input = abs(input);
//    while(input > 0){
//        remainderInt = input % 2;
//        binary.push_back(remainderInt);
//        input/=2;
//    }
//    for (int i : binary){
//        if (i ==0){
...
//    }
//    for(int i = 0; i < binary.size(); i ++){
//        endResult+= pow(2,binary.size()-1-i) * binary[binary.size()-1-i];
//    }
//    return endResult;
